...
public class ControlAreas {
    ...
    public ControlAreas(double T1, double T2, double T3, double T4, double T5, double T6, double Tf, double Tfout, double Qevp, double Qcon, double Qreg, double eff, double Hlat, double Hsen, double Hsup, double T1s, double PP, double SUP, double m, double Pref, double Tref, double P4, int ii, double H3, int compressor, int FON, double mf, double Pf, Session session) {
        ...
        double Hin, Test = 0, erro, DT, it, Hout, Q, Burbuja;

        if (SUP > 0) {
            if (FON == 1) {

                mf = mf / 18;//; %Dividi do peso molecular para pasar a kmol
                ControlH_Sistema hSistema = new ControlH_Sistema(Tf, Pf, Pref, Tref, 1, session);
                Hin = hSistema.getHL();
                Test = Tf - 2;
                erro = 1;
                DT = (Tf - T1s) / 2;
